...
export async function runPresenterAndNotesJourney(page: Page, baseURL: string): Promise<void> {
  ...
  await addAndPersistSpeakerNotes(editor, page);
  await addPresenterCloseSlide(page);
  await verifyPresentationMenuAndEditorShortcuts(editor, page);

  const presenterPage = await openPresenterView(page);
  await dismissPresenterIntro(presenterPage);

  await verifyPresenterNotesAndTools(presenterPage);
  await verifyPresenterNavigation(presenterPage);
}

async function addAndPersistSpeakerNotes(editor: EditorAppPage, page: Page): Promise<void> {
...
async function addPresenterCloseSlide(page: Page): Promise<void> {
...
async function verifyPresentationMenuAndEditorShortcuts(editor: EditorAppPage, page: Page): Promise<void> {
...
async function openPresenterView(page: Page): Promise<Page> {
...
async function dismissPresenterIntro(presenterPage: Page): Promise<void> {
...
async function verifyPresenterNotesAndTools(presenterPage: Page): Promise<void> {
  ...
  await expect(presenterPage.getByRole('region', { name: 'Remote control this presentation' })).toBeVisible();
  await presenterPage.getByRole('main', { name: 'Presenter view' }).click({ position: { x: 12, y: 12 } });
  await expect(presenterPage.getByRole('region', { name: 'Remote control this presentation' })).toBeHidden();
...
async function verifyPresenterNavigation(presenterPage: Page): Promise<void> {
