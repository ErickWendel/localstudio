...
test.describe('editor AI tools status journey', () => {
  test('checks local AI status surfaces without downloading or executing models', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /Download|Prepare|Create image/ }).first()).toBeVisible();
  });
});
