...
export async function runModelDownloadContractInBrowser(): Promise<ModelDownloadContractResult> {
  ...
  const imageEditingState = await browserSetup.downloadModel(modelSetupService.IMAGE_EDITING_MODEL_ID);
  const imageGenerationState = await browserSetup.downloadModel(imageGenerationModel.IMAGE_GENERATION_MODEL_ID);
