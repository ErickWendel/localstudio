...
export async function runPresenterKeyboardVideoJourney(page: Page, baseURL: string): Promise<void> {
  ...
  await addVideoAndSecondSlide(editor, page);
  await enterFullscreenPresentation(editor, page);

  const workspace = page.getByRole('region', { name: 'Canvas workspace' });
  await workspace.focus();
  await expect(page.getByText('1 / 2')).toBeVisible();

  const video = await prepareVideo(workspace);
  const shortcuts = await openKeyboardShortcuts(editor, page);

  await verifyVideoShortcuts(page, shortcuts, video);
  await verifySlideNavigationShortcuts(page, shortcuts);
  await verifyPauseAndExitShortcuts(page, shortcuts);
}

async function installFullscreenMock(page: Page): Promise<void> {
...
async function addVideoAndSecondSlide(editor: EditorAppPage, page: Page): Promise<void> {
...
async function enterFullscreenPresentation(editor: EditorAppPage, page: Page): Promise<void> {
...
async function prepareVideo(workspace: Locator): Promise<Locator> {
...
async function openKeyboardShortcuts(editor: EditorAppPage, page: Page): Promise<Locator> {
...
async function verifyVideoShortcuts(page: Page, shortcuts: Locator, video: Locator): Promise<void> {
...
async function verifySlideNavigationShortcuts(page: Page, shortcuts: Locator): Promise<void> {
...
async function verifyPauseAndExitShortcuts(page: Page, shortcuts: Locator): Promise<void> {
