...
type CanvasDesignPoint = {
  x: number;
  y: number;
};

async function getCanvasPoint(page: Page, point: CanvasDesignPoint) {
...
export const canvasTransformJourneyPage = {
  async resizeTextAndUseArrangeControls(page: Page, baseURL: string) {
  ...
  async cropImportedImage(page: Page, baseURL: string, testInfo: TestInfo) {
    const editor = new EditorAppPage(page, baseURL);
    await editor.gotoNewProject();
