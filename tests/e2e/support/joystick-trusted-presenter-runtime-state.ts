...
export const joystickTrustedPresenterRuntimeState = {
  async install(page: Page, config: JoystickTrustedPresenterInstallConfig): Promise<void> {
    await page.addInitScript(({ initialState }) => {
      ...
      function publishCommand(command: PresenterRemoteCommand) {
        commands.push(
      ...
      window.__LOCALSTUDIO_E2E_JOYSTICK_RUNTIME__ = {
        commands,
        getState: () => state,
        publishCommand,
      ...
      window.__LOCALSTUDIO_E2E_JOYSTICK__ = window.__LOCALSTUDIO_E2E_JOYSTICK_RUNTIME__;
    }, config);
  },
};
