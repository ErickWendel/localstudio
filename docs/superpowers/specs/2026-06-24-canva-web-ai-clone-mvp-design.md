...
- Selecting an image shows a LocalStudio.ai-styled image editing toolbar inspired by Canva, with `BG Remover`, `Flip`, and `Crop`. `Flip` persists as an undoable image element property. `Crop` enters a draggable crop-frame mode with edge/corner handles and persists the normalized image crop rect through the command/history flow.
- AI Tools panel is provider-neutral: configuration appears first for LLM model selection, translation model/target language, and image generation settings; one unified visible model list follows for image editing and image generation dependencies. LLM and translation backing model progress stays inside their configuration cards to avoid duplicate `needs download` rows. The old `Local Chrome AI` and `Cached Browser Models` groupings are removed.
...
- Use a prompt input below the main canvas to generate active-page slide structure, text organization, and basic layout changes through the browser Prompt API.
...
- AI Tools should be provider-neutral: configuration appears first, then one unified visible model list for image editing and image generation dependencies. LLM and translation backing model download/preparation progress belongs in the configuration cards. Remove the old `Local Chrome AI` and `Cached Browser Models` grouping copy.
...
- Generate active-page Prompt API slide layouts with mocked provider.
- Generate and apply Prompt API palette suggestions with mocked provider once palette generation is folded into the prompt flow.
