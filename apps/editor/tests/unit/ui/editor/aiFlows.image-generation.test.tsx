...
const { DeferredImageGenerationService, SlowImageGenerationService, createAppServices } =
  aiFlowTestFixtures;
...
describe('mocked AI image generation flows', () => {
  it('redirects create image prompt typing to AI Tools when image generation models are not ready', async () => {
    ...
    expect(screen.getByRole('article', { name: 'Image Generation Models' })).toHaveClass(
      'model-row-attention',
    );
    expect(
      screen.getByText('Download image generation models before creating images.'),
    ).toBeInTheDocument();
  });
  ...
  it('downloads image generation models before allowing create image prompts', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('article', { name: 'Image Generation Models' })).toHaveTextContent(
        'Ready',
      );
    });
  ...
  it('generates an image from create image mode and inserts it into the active slide', async () => {
    ...
    expect(capture).toHaveBeenCalledWith(
      postHogEvents.aiImageGenerated,
      expect.objectContaining(expectedModelProperties),
    );
    expect(capture).toHaveBeenCalledWith(
      postHogEvents.promptGeneratedImage,
      expect.objectContaining(expectedModelProperties),
    );
  });
  ...
  it('passes selected create image options to the generator', async () => {
    ...
    const generateImage = vi.fn(
      (_prompt: string, options?: ImageGenerationOptions): Promise<Asset> => {
